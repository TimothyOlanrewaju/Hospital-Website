...
<body>
  ...
  <tbody>
  	@foreach($data as $contact_admin)
    <tr align="center" style="padding:20px;">
      <td>{{$contact_admin->name}}</td>
      <td>{{$contact_admin->email}}</td>
      <td>{{$contact_admin->subject}}</td>
      <td>{{$contact_admin->message}}</td>
      <td>
      	<a class="btn btn-success" href="{{url('approved',$contact_admin->id)}}">Respond</a>
      </td>

      <td>
      	<a class="btn btn-danger" href="{{url('cancel',$contact_admin->id)}}">Decline</a>
      </td>
    </tr>
    @endforeach
