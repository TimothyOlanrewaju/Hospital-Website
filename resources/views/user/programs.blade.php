...
<section class="max-w-screen-2xl px-4 py-2">
    @foreach($update->chunk(3) as $chunk)
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-2 lg:gap-3">
        @foreach($chunk as $post)
        <div class="mx-auto p-2 mt-2 text-center text-gray-900 bg-white border border-gray-100 rounded-lg shadow-lg shadow-green-500 dark:border-gray-600 xl:p-8 dark:bg-gray-800 dark:text-white">
            <div class="w-90 h-40 lg:h-56 overflow-hidden">
                <img src="/progimage/{{$post->image}}" alt="" class="object-cover w-full h-full">
            </div>
